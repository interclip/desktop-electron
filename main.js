...
if (setupEvents.handleSquirrelEvent()) { // squirrel event handled and app will exit in 1000ms, so don't do anything else
    return;
}

const endpoint = "https://interclip.app";
...
function createWindow() { // Create the browser window.
        ...
        axios.get(`${endpoint}/includes/api?url=${url}`).then((res) => {
            ...
            event.sender.send("asynchronous-reply", code);
                ...
                if (err.toString().includes("429")) {
                  dialog.showErrorBox("Slow down, backaroo!", `Rate limit exceeded. Try again in a minute`);
                } else {
                  dialog.showErrorBox("There is an error with your URL", `${err}`);
                }
        });
    });
    ...
    ipcMain.on("recieve-code", (event, code) => {
        if (code.length !== 5) {
            dialog.showErrorBox("Error with your code", `This code should be 5 characters long, not ${
                code.length
            }, please correct that.`);
        } else {
            axios.get(`${endpoint}/includes/get-api?code=${code}`).then((res) => {
                const url = res.data.result;
                // send message to index.html
                event.sender.send("url-reply", url);
            }).catch((err) => {
                dialog.showErrorBox(`Seems like... that isn't a code at all!`, `(${err})`);
            });
        }
    });
